...
// دالة نسخ الرابط لصفحة المقال الحالية (متاحة عالمياً)
...
// دالة نسخ رابط سؤال محدد من القائمة الرئيسية
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (content) {
        ...
        if (share) {
            // تم دمج خيارات المشاركة مع إضافة زر Twitter/X وتفعيل زر النسخ المباشر
            share.innerHTML = `
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <!-- Twitter / X -->
                    <a href="https://twitter.com/intent/tweet?text=${encodeURIComponent(document.title)}&url=${encodeURIComponent(window.location.href)}" target="_blank" class="w-10 h-10 rounded-xl bg-slate-900/10 dark:bg-white/10 hover:bg-slate-900/20 dark:hover:bg-white/20 flex items-center justify-center text-slate-800 dark:text-slate-100 transition-transform hover:-translate-y-0.5" title="Share on X">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    </a>
                    
                    <!-- Copy Link Button (يعمل الآن بشكل صحيح) -->
                    <button onclick="window.copyCurrentUrl()" class="px-4 h-10 rounded-xl bg-indigo-600/10 dark:bg-indigo-400/10 hover:bg-indigo-600/20 dark:hover:bg-indigo-400/20 flex items-center justify-center text-indigo-600 dark:text-indigo-400 text-xs font-bold transition-all hover:-translate-y-0.5 gap-2" title="Copy Direct Link">
